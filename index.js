...
function init() {
  ...
  container.addEventListener('dblclick', addCube, false);

  function addCube(event) {
    var ge = new BoxGeometry(2, 2, 2);
    var material = new MeshBasicMaterial({ color: 0x000000 });
    var ms = new Mesh(ge, material);

    var raycaster = new Raycaster();
    var mouse = new Vector2();

    mouse.x = (event.clientX / renderer2.domElement.width) * 2 - 1;
    mouse.y = -(event.clientY / renderer2.domElement.height) * 2 + 1;
    raycaster.setFromCamera(mouse, camera);
    var vec = raycaster.ray.intersectPlane(new Plane(new Vector3(0, 1, 0)));
    ms.position.x = vec.x;
    ms.position.y = vec.y;
    ms.position.z = vec.z;
    material_scene.add(ms);
    console.log(vec.x, vec.y, vec.z);
    console.log(material_scene);
  }
}
